...
//     echo $this->getKey();
  ...
  function getUrlLink() {
    // if ($this->hasField('url')) return ' <a'.(BIBTEXBROWSER_BIB_IN_NEW_WINDOW?' target="_blank" ':'').' href="'.$this->getField('url').'" style="color:red; font-weight:400">[pdf]</a>';
    return '';
  }
...
function DefaultBibliographyStyle(&$bibentry) {
 ...
 if ($bibentry->hasField('series')) $entry[] =  "".$bibentry->getField("series");

  if ($bibentry->hasField('volume')) $entry[] =  "volume ".$bibentry->getField("volume");

  if ($bibentry->hasField('number')) $entry[] =  "number ".$bibentry->getField("number");

  if ($bibentry->hasField('issue')) $entry[] =  "issue ".$bibentry->getField("issue");

  if ($bibentry->hasField('pages')) $entry[] =  "pp. ".$bibentry->getField("pages");
...
function query2title(&$query) {
    ...
    if (isset($headers[Q_AUTHOR])) $headers[Q_AUTHOR] = 'Publications/Manuscripts of '.htmlspecialchars($_GET[Q_AUTHOR]);
    return join(' &amp; ',$headers);
}
}
...
class Dispatcher {
  ...
  function frameset() {    ?>
    ...
    <meta name="generator" content="bibtexbrowser v20130328" />
    <meta http-equiv="Content-Type" content="text/html; charset=<?php echo ENCODING ?>"/>
      <title>Publications/Manuscripts of Christoph Benzm&uuml;ller</title>
    </head>
    <frameset cols="20%,*">
